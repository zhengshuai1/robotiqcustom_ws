...
class BaseArmController(object):
    """Nachi robot grasp Control"""

    def __init__(self):
        super(BaseArmController, self).__init__()
        ...
        self.end_y_pos = -0.2
        self.wait_z = 0.27
        self.reset_x, self.reset_y, self.reset_z = [0.34, self.wait_y, 0.28]
        ...
        self.min_tcp_z = 0.178   # 0.177 0.17634
        ...
        if self.press_depth > 0.009 or self.min_tcp_z < 0.17634:
    ...
    def plan_tracking_trajectories(self, group_name, grasp, label, meeting_point_y=None):
        ...
        T_base_grasp = grasp
        T_base_grasp.translation[1] = self.wait_y  # wait y pos
        ...
        T_grasp_retreat = Transform(Rotation.identity(), [0.0, 0.0, self.pregrasp_dz])  # global frame Z axis aim up, so z is
        ...
        T_base_middle_place, T_base_place = self.select_place_pose(label)
    ...
    def move_to_wait(self, T_lbase_obj):
        ...
        wait_pose.translation = [T_lbase_obj.translation[0], self.wait_y, self.wait_z]
    ...
    def wait_picking(self, T_lbase_obj, img_time_stamp):
        ...
        dis = T_lbase_obj.translation[1]-self.wait_y
    ...
    def is_object_out_of_region(self, mat_grasp, img_time_stamp):
        pre_wait_y = self.wait_y + self.conveyor_speed * self.t_pre_wait_t  # 0.224m
        ...
        # new_cur_y = cur_y - self.conveyor_speed * self.gripper_time  # gripper activate time 0.3 s
        ...
        if meeting_point_y is None or meeting_point_y < end_y_pos:
            ...
            return False, None
        elif self.end_y_pos <= meeting_point_y and cur_y <= pre_wait_y:
            if cur_y <= self.wait_y:
    ...
    def do_picking(self, req):
        ...
        min_tcp_z = self.min_tcp_z
        if mat_grasp[2, 3] < min_tcp_z:
            mat_grasp[2, 3] = min_tcp_z  # z
            rospy.loginfo('z %.8f is wrong, error is %.8f', now_z, min_tcp_z-z_value)
